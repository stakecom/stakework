...
`qa/pull-tester/test_framework/test_framework.py:StakeWorkTestFramework.main`.
...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_STAKEWORKD' not in vars():
    ENABLE_STAKEWORKD=0
...
#Set env vars
if "STAKEWORKD" not in os.environ:
    os.environ["STAKEWORKD"] = BUILDDIR + '/src/stakeworkd' + EXEEXT
if "STAKEWORKCLI" not in os.environ:
    os.environ["STAKEWORKCLI"] = BUILDDIR + '/src/stakework-cli' + EXEEXT

if EXEEXT == ".exe" and "-win" not in opts:
    # https://github.com/stakework/stakework/commit/d52802551752140cf41f0d9a225a43e84404d3e9
    # https://github.com/stakework/stakework/pull/5677#issuecomment-136646964
    print("Win tests currently disabled by default.  Use -win option to enable")
    sys.exit(0)

if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_STAKEWORKD == 1):
    print("No rpc tests to run. Wallet, utils, and stakeworkd must all be enabled")
    sys.exit(0)
...
class RPCCoverage(object):
    ...
    commands per `stakework-cli help` (`rpc_interface.txt`).
