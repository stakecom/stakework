...
The RPM spec file provided here is for StakeWork-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://stakework.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[StakeWork git repository](https://github.com/stakework/stakework) but are not part of
...
Currently only `Source10` is used, for the example `stakework.conf` file.
...
in the stakework git repository. At present only an SVG version of the StakeWork
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the stakework
...
needed when 0.12.1 is released, a proper fix is already in the StakeWork git
master branch.
...
idea to build StakeWork Core against a static version of that library compiled
...
The `stakework.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `stakework-server` package. At this point, most RPM based
...
The `stakework-server` package should have SELinux support. How to properly do
...
StakeWork-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/stakework/). He has been
packaging StakeWork for Fedora at least since 2012.
...
4. I renamed the `stakework` package that contains the Qt GUI to `stakework-core` as
that appears to be how the general population refers to it, in contrast to
`stakework-xt` or `stakework-classic`. I wanted to make sure the general population
