...
###   This script attempts to download the signature file SHA256SUMS.asc from stakework.org
...
WORKINGDIR="/tmp/stakework"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://stakework.org/bin/"
VERSIONPREFIX="stakework-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'stakework-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the StakeWork Core binary release signing key installed?"
   fi
