...
const int STAKEWORK_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString STAKEWORK_IPC_PREFIX("stakework:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
const char* NIP01_MESSAGE_SIGNEDMSG = "SignedMsg";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/stakework-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/stakework-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/stakework-paymentrequest";
const char* NIP01_MIMETYPE_SIGNEDMSG = "application/stakework-signedmsg";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the stakework: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(STAKEWORK_IPC_PREFIX, Qt::CaseInsensitive)) // stakework: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseStakeWorkURI(arg, &r) && !r.address.isEmpty())
            {
                CStakeWorkAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r: savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(STAKEWORK_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(STAKEWORK_IPC_CONNECT_TIMEOUT);
    ...
    model(0)
{
    ...
    // on Mac: sent when you click stakework: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            LogPrintf("Cannot start stakework: click-to-pay handler\n");
        }
        else {
...
//
// OSX-specific way of handling stakework: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in stakework: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("stakework://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'stakework://' is not a valid URI. Use 'stakework:' instead."),
                       CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(STAKEWORK_IPC_PREFIX, Qt::CaseInsensitive)) // stakework: URI
    {
        ...
        if (uri.hasQueryItem("m") && uri.hasQueryItem("a")) // sign message URI
        {
            ...
            CStakeWorkAddress addr(uri.queryItemValue("a").toStdString());
            if (!addr.IsValid())
            {
            ...
            QUrl fetchUrlConstructed(s.mid(STAKEWORK_IPC_PREFIX.length()));
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseStakeWorkURI(s, &recipient))
            {
              ...
              if (!address.IsValid()) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid StakeWork address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for(const PAIRTYPE(CScript, CAmount)& sendingTo: sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CStakeWorkAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom stakework addresses are not supported
        ...
        // StakeWork amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(StakeWorkUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
