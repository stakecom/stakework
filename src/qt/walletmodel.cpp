...
bool WalletModel::validateAddress(const QString &address)
{
  ...
  CStakeWorkAddress addressParsed(address_str);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    for(const SendCoinsRecipient &rcp: recipients)
    {
        ...
        {   // User-entered stakework address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CStakeWorkAddress(rcp.address.toStdString()).Get());
        ...
        for(const CRecipient &rcp: transaction.vecSend)
        {
          ...
          for(const CWalletTx newTx: transaction.vTransactions)
          {
            ...
            for(const SendCoinsRecipient &rcp: transaction.getRecipients())
            {
              ...
              else if (!rcp.message.isEmpty()) // Message from normal stakework:URI (stakework:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CStakeWorkAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    for(const COutput& out: vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CStakeWorkAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CStakeWorkAddress(sAddress).Get();
