...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    processSendCoinsReturn(prepareStatus,
        StakeWorkUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    if (currentTransaction.fSpendsColdStaking && (!model->getOptionsModel()->getCoinControlFeatures() ||
        (model->getOptionsModel()->getCoinControlFeatures() && !CStakeWorkAddress(CoinControlDialog::coinControl->destChange).IsColdStakingAddress(Params()))))
    {
    ...
    const SendCoinsRecipient &rcp = currentTransaction.recipients.first();
    {
        ...
        QString amount = "<b>" + StakeWorkUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount);
        amount.append("</b>");
        ...
        if(txFee)
        {
            ...
            questionString.append(StakeWorkUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    // Check if we have selected a display unit that is not XSW
    if (model->getOptionsModel()->getDisplayUnit() != StakeWorkUnits::XSW)
        alternativeUnits.append(StakeWorkUnits::formatHtmlWithUnit(StakeWorkUnits::XSW, totalAmount));

    // Check if we have selected a display unit that is not BTC
    if (model->getOptionsModel()->getDisplayUnit() != StakeWorkUnits::BTC)
        alternativeUnits.append(StakeWorkUnits::formatHtmlWithUnit(StakeWorkUnits::BTC, totalAmount));

    questionString.append(tr("Total Amount %1")
        .arg(StakeWorkUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(StakeWorkUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    ...
    CStakeWorkAddress addr = CStakeWorkAddress(text.toStdString());
    ...
    {
        ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid StakeWork address"));
