...
{
    ui->setupUi(this);
    GUIUtil::setupAddressWidget(ui->lineEditStakeworkAddress, this);
    ...
    QString warning = tr("By submitting the proposal a contribution of %1 XSW to the Community Fund will occur from your wallet.").arg(QString::fromStdString(fee));
...
{
    bool isValid = true;
    if(!ui->lineEditStakeworkAddress->isValid() || (ui->lineEditStakeworkAddress->text() == QString("")))
    {
        // Styling must be done manually as an empty field returns valid (true)
        ui->lineEditStakeworkAddress->setStyleSheet(STYLE_INVALID);
        ui->lineEditStakeworkAddress->setValid(false);
        isValid = false;
    }
...
void CommunityFundCreateProposalDialog::click_pushButtonCreateProposal()
{
    ...
    if(this->validate())
    {
        ...
        // Address
        string Address = ui->lineEditStakeworkAddress->text().toStdString().c_str();
        ...
        if (curBalance <= GetConsensusParameter(Consensus::CONSENSUS_PARAM_PROPOSAL_MIN_FEE, view)) {
            QMessageBox msgBox(this);
            string fee = FormatMoney(GetConsensusParameter(Consensus::CONSENSUS_PARAM_PROPOSAL_MIN_FEE, view));
            std::string str = tr("You require at least %1 XSW mature and available to create a proposal\n").arg(QString::fromStdString(fee)).toStdString();
            msgBox.setText(tr(str.c_str()));
            msgBox.addButton(tr("Ok"), QMessageBox::AcceptRole);
            msgBox.setIcon(QMessageBox::Warning);
