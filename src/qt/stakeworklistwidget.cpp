...
#include "stakeworklistwidget.h"

StakeWorkListWidget::StakeWorkListWidget(QWidget *parent, QString title, ValidatorFunc validator) :
    QWidget(parent),
...
void StakeWorkListWidget::onRemove()
{
    listWidget->takeItem(listWidget->row(listWidget->currentItem()));
}

void StakeWorkListWidget::onSelect(QListWidgetItem* item)
{
    removeBtn->setVisible(item != nullptr);
}

void StakeWorkListWidget::onInsert()
{
...
QStringList StakeWorkListWidget::getEntries()
{
    QStringList ret;
