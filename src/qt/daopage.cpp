    ...
    nCurrentFilter2(FILTER_ALL)
{
    ...
    proposalsBtn = new StakeWorkPushButton(tr("Fund Proposals"));
    paymentRequestsBtn = new StakeWorkPushButton(tr("Payment Requests"));
    consultationsBtn = new StakeWorkPushButton(tr("Consultations"));
    deploymentsBtn = new StakeWorkPushButton(tr("Deployments"));
    consensusBtn = new StakeWorkPushButton(tr("Consensus Parameters"));
...
void DaoPage::initialize(CProposalMap proposalMap, CPaymentRequestMap paymentRequestMap, CConsultationMap consultationMap, CConsultationAnswerMap consultationAnswerMap, int unit, bool updateFilterIfEmpty)
{
    ...
    for (auto& it: proposalMap)
    {
        ...
        ProposalEntry p = {
            ...
            StakeWorkUnits::formatWithUnit(unit, proposal.nAmount, false, StakeWorkUnits::separatorAlways),
            StakeWorkUnits::formatWithUnit(unit, proposal.nAmount - proposal.GetAvailable(coins), false, StakeWorkUnits::separatorAlways),
    ...
    for (auto& it: paymentRequestMap)
    {
        ...
        PaymentRequestEntry p = {
            ...
            QString::fromStdString(proposal.strDZeel).left(150) + (proposal.strDZeel.size() > 150 ? "..." : ""),
            "#6666ff",
            StakeWorkUnits::formatWithUnit(unit, prequest.nAmount, false, StakeWorkUnits::separatorAlways),
