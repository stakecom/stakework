...
bool GetAddressesPage::validatePage()
{
    ...
    CStakeWorkAddress stakingAddress(stakingAddressStr.toStdString());
    CStakeWorkAddress spendingAddress(spendingAddressStr.toStdString());
...
void ColdStakingAddressPage::initializePage()
{
    ...
    coldStakingAddress = QString::fromStdString(CStakeWorkAddress(stakingKeyID, spendingKeyID).ToString());

#ifdef USE_QRCODE
    QString uri = "stakework:" + coldStakingAddress;
    image->setText("");
    if(!uri.isEmpty())
    {
        // limit URI length
