    ...
    comboAddress(new QComboBox)
{
    ...
    for (const COutput& out: vCoins)
    {
        ...
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, dest)){
            CStakeWorkAddress address(dest);
    ...
    {
        comboAddress->insertItem(i++, it.first +" ("+ QString::fromStdString(FormatMoney(it.second)) +" XSW)",it.first);
    }
...
void SplitRewardsDialog::showFor(QString sin)
{
    ...
    for (auto &key: j.keys())
    {
        CStakeWorkAddress address(key.toStdString());
        double amount;
    ...
    strDesc->setText(tr("For each block, %1 XSW will go to the Community Fund, %2 and %3 will be accumulated in your own address").arg(QString::fromStdString(FormatMoney(nCFundContribution)), descs.join(", "), QString::fromStdString(FormatMoney(PercentageToNav(availableAmount)))));
...
void SplitRewardsDialog::onAdd()
{
    ...
    if (!CStakeWorkAddress(address.toStdString()).IsValid())
    {
        QMessageBox msgBox(this);
        msgBox.setText(tr("Invalid StakeWork Address"));
        msgBox.addButton(tr("Ok"), QMessageBox::AcceptRole);
        msgBox.setIcon(QMessageBox::Warning);
        msgBox.setWindowTitle("Error");
        msgBox.exec();
