    ...
    secDelay(secDelay)
{
    ...
    // Amount label
    QSettings settings;
    ui->labelRequestedAmount->setText(QString("%1 XSW / ").arg(proposal->nAmount/COIN).append("%1 EUR / ").arg(proposal->nAmount / settings.value("eurFactor", 0).toFloat()).append("%2 USD / ").arg(proposal->nAmount / settings.value("usdFactor", 0).toFloat()).append("%3 BTC").arg(proposal->nAmount / settings.value("btcFactor", 0).toFloat()));
    ...
    string warning = tr("By submitting the payment request a contribution of %1 XSW to the Community Fund will occur from your wallet.").arg(QString::fromStdString(fee)).toStdString();
    ...
    secDelay(secDelay)
{
    ...
    // Amount label
    QSettings settings;
    ui->labelRequestedAmount->setText(QString("%1 XSW / ").arg(prequest->nAmount/COIN).append("%1 EUR / ").arg(prequest->nAmount / settings.value("eurFactor", 0).toFloat()).append("%2 USD / ").arg(prequest->nAmount / settings.value("usdFactor", 0).toFloat()).append("%3 BTC").arg(prequest->nAmount / settings.value("btcFactor", 0).toFloat()));
