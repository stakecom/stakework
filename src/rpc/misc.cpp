...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total stakework balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"stakeworkaddress\"\n"
            "\nReturn information about the given stakework address.\n"
            "\nArguments:\n"
            "1. \"stakeworkaddress\"     (string, required) The stakework address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"stakeworkaddress\", (string) The stakework address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"stakeworkaddress\", (string) The stakework staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"stakeworkaddress\", (string) The stakework spending address part of a cold staking address\n"
            "  \"votingaddress\" : \"stakeworkaddress\", (string) The stakework voting address part of a cold staking v2 address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            CStakeWorkAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CStakeWorkAddress spendingAddress;
            ...
            CStakeWorkAddress spendingAddress;
            address.GetSpendingAddress(spendingAddress);
            CStakeWorkAddress votingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: StakeWork address and we have full public key:
        CStakeWorkAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are stakework addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) stakework address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CStakeWorkAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CStakeWorkAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"stakeworkaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"stakeworkaddress\"  (string, required) The stakework address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CStakeWorkAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CStakeWorkAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CStakeWorkAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
