...
const char * const STAKEWORK_CONF_FILENAME = "stakework.conf";
const char * const STAKEWORK_PID_FILENAME = "stakework.pid";
...
#else
    const char* pszModule = "stakework";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StakeWork
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StakeWork
    // Mac: ~/Library/Application Support/StakeWork
    // Unix: ~/.stakework
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StakeWork4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/StakeWork4";
#else
    // Unix
    return pathRet / ".stakework4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", STAKEWORK_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No stakework.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override stakework.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", STAKEWORK_PID_FILENAME));
