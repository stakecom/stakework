...
UniValue createrawscriptaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the StakeWork address for the specified raw hex script.\n"
            "\nArguments:\n"
            "1. \"hex script\"        (string) The hex script to encode in the address.\n"
            "\nResult:\n"
            "\"stakeworkaddress\"    (string) The  stakework address\n"
            "\nExamples:\n"
    ...
    CStakeWorkAddress address(script);
...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new StakeWork address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"stakeworkaddress\"    (string) The new stakework address\n"
            "\nExamples:\n"
    ...
    return CStakeWorkAddress(keyID).ToString();
}
...
UniValue getcoldstakingaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2)
        throw runtime_error(
            ...
            "Arguments:\n"
            "1. \"stakingaddress\"  (string, required) The stakework staking address.\n"
            "2. \"spendingaddress\" (string, required) The stakework spending address.\n\n"
            "3. \"voting\"          (string, optional) The stakework voting address.\n\n"
            "\nExamples:\n"
    ...
    CStakeWorkAddress stakingAddress(params[0].get_str());
    CKeyID stakingKeyID;
    if (!stakingAddress.IsValid() || !stakingAddress.GetKeyID(stakingKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Staking address is not a valid StakeWork address");

    CStakeWorkAddress spendingAddress(params[1].get_str());
    CKeyID spendingKeyID;
    if (!spendingAddress.IsValid() || !spendingAddress.GetKeyID(spendingKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Spending address is not a valid StakeWork address");

    spendingAddress.GetKeyID(spendingKeyID);

    if (params.size() == 3)
    {
        CStakeWorkAddress votingAddress(params[2].get_str());
        CKeyID votingKeyID;
        if (!votingAddress.IsValid() || !votingAddress.GetKeyID(votingKeyID))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Voting address is not a valid StakeWork address");

        votingAddress.GetKeyID(votingKeyID);

        return CStakeWorkAddress(stakingKeyID, spendingKeyID, votingKeyID).ToString();
    }

    return CStakeWorkAddress(stakingKeyID, spendingKeyID).ToString();
}


CStakeWorkAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CStakeWorkAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current StakeWork address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"stakeworkaddress\"   (string) The account stakework address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new StakeWork address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"stakeworkaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"stakeworkaddress\"  (string, required) The stakework address to be associated with an account.\n"
    ...
    CStakeWorkAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StakeWork address");
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"stakeworkaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"stakeworkaddress\"  (string, required) The stakework address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"stakeworkaddress\"  (string) a stakework address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    for(const PAIRTYPE(CStakeWorkAddress, CAddressBookData)& item: pwalletMain->mapAddressBook)
    {
        const CStakeWorkAddress& address = item.first;
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw runtime_error(
            "sendtoaddress \"stakeworkaddress\" amount ( \"comment\" \"comment-to\" \"strdzeel\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"stakeworkaddress\"  (string, required) The stakework address to send to.\n"
            ...
            "                             The recipient will receive less stakeworks than you enter in the amount field.\n"
            "\nResult:\n"
    ...
    CStakeWorkAddress address(address_str);
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StakeWork address");
...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 4)
        throw runtime_error(
            "createproposal \"stakeworkaddress\" \"amount\" duration \"desc\" ( fee dump_raw )\n"
            "\nCreates a proposal for the community fund. Min fee of " + FormatMoney(GetConsensusParameter(Consensus::CONSENSUS_PARAM_PROPOSAL_MIN_FEE, view)) + "XSW is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"stakeworkaddress\"       (string, required) The stakework address where coins would be sent if proposal is approved.\n"
    ...
    CStakeWorkAddress address("NQFqqMUD55ZV3PJEJZtaKCsQmjLT6JkjvJ"); // Dummy address
    ...
    if (!paddress.IsValid())
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Stakework address for payment");

    CStakeWorkAddress oaddress(ownerAddress);
    if (!oaddress.IsValid())
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Stakework address for the owner");
...
UniValue proposeconsensuschange(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || !params[0].isNum() || !params[1].isNum())
        throw runtime_error(
            ...
            "\nCreates a proposal to the DAO for changing a consensus paremeter. Min fee of " + FormatMoney(nMinFee) + "XSW is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
...
UniValue createconsultation(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1)
        throw runtime_error(
            ...
            "\nCreates a consultation for the DAO. Min fee of " + FormatMoney(GetConsensusParameter(Consensus::CONSENSUS_PARAM_CONSULTATION_MIN_FEE, view)) + "XSW is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            ...
            "\"  strDZeel: string }\"            (string) The attached strdzeel property.\n"
            "\nExamples:\n"
            + HelpExampleCli("createconsultation", "\"Who should be the CEO of StakeWork? /s\" 1 1")
            + HelpExampleCli("createconsultation", "\"How much should StakeWork's CEO earn per month? /s\" 1000 5000 true")
        );
...
UniValue createconsultationwithanswers(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2)
        throw runtime_error(
            ...
            "\"  strDZeel: string }\"            (string) The attached strdzeel property.\n"
            "\nExamples:\n"
            + HelpExampleCli("createconsultationwithanswers", "\"Who should be the CEO of StakeWork? /s\" \"[\\\"Craig Wright\\\",\\\"Loomdart\\\"]\"")
        );
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3)
        throw runtime_error(
            ...
            "\nCreates a proposal to withdraw funds from the community fund. Fee: 0.0001 XSW\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    CStakeWorkAddress address(proposal.GetOwnerAddress());

    if(!address.IsValid())
        throw JSONRPCError(RPC_TYPE_ERROR, "Address of the proposal is not a valid StakeWork address.");
    ...
    std::string Secret = sRandom + "I kindly ask to withdraw " +
            std::to_string(nReqAmount) + "XSW from the proposal " +
...
UniValue proposeanswer(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2)
        throw runtime_error(
            ...
            "\nProposes an answer for an already existing consultation of the DAO. Min fee of " + FormatMoney(GetConsensusParameter(Consensus::CONSENSUS_PARAM_CONSULTATION_ANSWER_MIN_FEE, view)) + "XSW is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
...
UniValue donatefund(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "                             The fund will receive less stakeworks than you enter in the amount field.\n"
            "\nResult:\n"
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    for(set<CTxDestination> grouping: pwalletMain->GetAddressGroupings())
    {
        ...
        for(CTxDestination address: grouping)
        {
            ...
            addressInfo.push_back(CStakeWorkAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CStakeWorkAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CStakeWorkAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"stakeworkaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"stakeworkaddress\"  (string, required) The stakework address to use for the private key.\n"
    ...
    CStakeWorkAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"stakeworkaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given stakeworkaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"stakeworkaddress\"  (string, required) The stakework address for transactions.\n"
    ...
    // StakeWork address
    CStakeWorkAddress address = CStakeWorkAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StakeWork address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tostakeworkaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a stakework address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tostakeworkaddress\"  (string, required) The stakework address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CStakeWorkAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid StakeWork address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The stakework address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less stakeworks than you enter in their corresponding amount field.\n"
    ...
    for(const string& name_: keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid StakeWork address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a StakeWork address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of stakework addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) stakework address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"stakeworkaddress\"  (string) A stakework address associated with the keys.\n"
    ...
    return CStakeWorkAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    return CStakeWorkAddress(w.result).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CStakeWorkAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    for(const PAIRTYPE(CStakeWorkAddress, CAddressBookData)& item: pwalletMain->mapAddressBook)
    {
        ...
        map<CStakeWorkAddress, tallyitem>::iterator it = mapTally.find(address);
    ...
    if (fAllAccounts || (account == strAccount))
    {
        ...
        entry.pushKV("canStake", (::IsMine(*pwalletMain, r.destination) & ISMINE_STAKABLE ||
                                          (::IsMine(*pwalletMain, r.destination) & ISMINE_SPENDABLE &&
                                           !CStakeWorkAddress(r.destination).IsColdStakingAddress(Params()) &&
                                           !CStakeWorkAddress(r.destination).IsColdStakingv2Address(Params()))) ? true : false);
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"stakeworkaddress\",    (string) The stakework address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"stakeworkaddress\",    (string) The stakework address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"stakeworkaddress\",   (string) The stakework address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending stakeworks\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending stakework\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"stakeworkaddress\" \"test message\"") +
    ...
    StartShutdown();
    return _("wallet encrypted; StakeWork server stopping, restart to run with encrypted wallet.");
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending stakeworks.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of stakework addresses to filter\n"
            "    [\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the stakework address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CStakeWorkAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid StakeWork address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    for(const COutput& out: vecOutputs) {
        ...
        if (fValidAddress) {
            entry.pushKV("address", CStakeWorkAddress(address).ToString());
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"     (string, optional, default pool address) The stakework address to receive the change\n"
    ...
    if (params.size() > 1) {
      if (params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CStakeWorkAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid stakework address");
...
UniValue resolveopenalias(const UniValue& params, bool fHelp)
{
  ...
  if ((fHelp || params.size() != 1))
      throw runtime_error(
          ...
          "\nResolves the given OpenAlias address to a StakeWork address.\n"
          "\nArguments:\n"
          "1. \"address\"    (string) The OpenAlias address.\n"
          "\nExamples:\n"
          "\nGet information about an OpenAlias address\n"
          + HelpExampleCli("resolveopenalias", "\"donate@stakework.org\"")
      );
...
UniValue listproposals(const UniValue& params, bool fHelp)
{
    ...
    if(view.GetAllProposals(mapProposals))
    {
        for (CProposalMap::iterator it = mapProposals.begin(); it != mapProposals.end(); it++)
        {
            ...
            if (showMine)
            {
                CTxDestination address(CStakeWorkAddress(proposal.GetOwnerAddress()).Get());
                isminefilter mine = IsMine(*pwalletMain, address);
