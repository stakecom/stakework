...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key, CScript& kernelScriptPubKey)
{
	...
	if(!fKernelFound && pindexPrev == chainActive.Tip())
        {
    ...
    std::map<CStakeWorkAddress, double> splitMap;
    double nAccumulatedFee = 0.0;

    CStakeWorkAddress poolFeeAddress(GetArg("-pooladdress", ""));
    ...
    else if (GetArg("-stakingaddress", "") != "")
    {
        ...
        if (stakingAddress.read(GetArg("-stakingaddress", "")))
        {
            try {
                ...
                if (find_value(addressMap, CStakeWorkAddress(key.GetPubKey().GetID()).ToString()).isStr())
                    lookForKey = CStakeWorkAddress(key.GetPubKey().GetID()).ToString();

                if (find_value(addressMap, CStakeWorkAddress(key.GetPubKey().GetID()).ToString()).isObject())
                {
                    find_value(addressMap, CStakeWorkAddress(key.GetPubKey().GetID()).ToString()).getObjMap(splitObject);
                ...
                {
                    address = CStakeWorkAddress(find_value(addressMap, lookForKey).get_str());
                ...
                else if(find_value(addressMap, lookForKey).isObject())
                {
                    ...
                    for ( const auto &pair : splitObject ) {
                        address = CStakeWorkAddress(pair.first);
        ...
        {
            address = CStakeWorkAddress(GetArg("-stakingaddress", ""));
            if (address.IsValid()) {
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CStakeWorkAddress(CScriptID(redeemScript)).ToString();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-stakework-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CStakeWorkAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CStakeWorkAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CStakeWorkAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CStakeWorkAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CStakeWorkAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CStakeWorkAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CStakeWorkAddress(dest).ToString(), key);
}
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-stakingaddress", strprintf(_("Specify a customised stakework address to accumulate the staking rewards.")));
...
std::string CWallet::formatDisplayAmount(CAmount amount) {
    ...
    string xsw_amount = n.str();
    if(xsw_amount.at(xsw_amount.length()-1) == '.') {
        xsw_amount = xsw_amount.substr(0, xsw_amount.size()-1);
    }
    xsw_amount.append(" XSW");
